...
class LoginDataHandler:
    ...
    def handle_data(self, data_decode):
        ...
        while True:
            ...
            if not handler:
                common.log_error("[login] unknow packet type",
                    data[:2].encode("hex"), data[2:].encode("hex"))
                return
            try:
                handler(self, data_io)
            except:
                common.log_error("[login] handle_data error:", data.encode("hex"))
    ...
    def do_001f(self, data_io):
        ...
        common.log("[login]", "login", username, password_sha1)
    ...
    def do_00a0(self, data_io):
        ...
        common.log("[login] create character:", "num", num, "name", name,
    ...
    def do_00a5(self, data_io):
        ...
        common.log("[login] remove character", "num", num,
        ...
        try:
            ...
            with self.user.lock:
                ...
                with users.user_list_lock:
                    common.log("[login] remove pc id", p.id)
    ...
    def do_00a7(self, data_io):
        #キャラクター選択
        num = io_unpack_byte(data_io)
        common.log("[login] select character", num)
        with self.user.lock:
    ...
    def do_002a(self, data_io):
        self.send("002b")
        #キャラ番号通知
        common.log("[login]", "request friend list")
        if self.pc:
...
LoginDataHandler.name_map = common.get_name_map(LoginDataHandler.__dict__, "do_")