...
def make(data_type, *args):
    ...
    if data_value is None:
        common.log_error("packet make error:", data_type, args)
        return ""
...
def make_0020(user, _type):
    ...
    if _type == "loginsucess":
        return "\x00\x00\x00\x00"+pack_int(user.user_id)+"\x00\x00\x00\x00"*2+"\x00"
    elif _type == "loginfaild":
        return "\xFF\xFF\xFF\xFE"+pack_int(user.user_id)+"\x00\x00\x00\x00"*2+"\x00"
    elif _type == "isonline":
        return "\xFF\xFF\xFF\xFB"+pack_int(user.user_id)+"\x00\x00\x00\x00"*2+"\x00"
    elif _type == "logoutstart":
        return "\x00"
    elif _type == "logoutcancel":
        return "\xF9"
    else:
        common.log_error("make_0020: type not exist", _type)
...
def make_00a1(_type):
    ...
    if _type == "sucess":
        ...
        common.log_error("make_00a1: type not exist", _type)
...
def make_0033(_type, _name, _addr):
    """接続先通知要求(ログインサーバ/0032)の応答"""
    if _type == "ping":
        result = ""
    elif _type == "launch":
        result = pack_str(_name)
        result += pack_str(_addr)
    else:
        result = "\x09"
...
def make_09e9(pc):
    ...
    l_s_motion = pack_item_unsigned_byte_attr(
        item_left, "s_motion", common.LEFT_TYPE_LIST
    )
    #左手モーション 両手
    l_d_motion = pack_item_unsigned_byte_attr(
        item_left, "d_motion", common.LEFT_TYPE_LIST
    )
    #右手モーション 片手
    r_s_motion = pack_item_unsigned_byte_attr(
        item_right, "s_motion", common.RIGHT_TYPE_LIST
    )
    #右手モーション 両手
    r_d_motion = pack_item_unsigned_byte_attr(
        item_right, "d_motion", common.RIGHT_TYPE_LIST
    )
    #頭
    result += pack_pict_id(item_head, common.HEAD_TYPE_LIST)
    #頭アクセサリ
    result += pack_pict_id(item_head, common.ACCESORY_HEAD_TYPE_LIST)
    #顔
    result += pack_pict_id(item_face, common.FULLFACE_TYPE_LIST)
    #顔アクセサリ
    result += pack_pict_id(item_face, common.ACCESORY_FACE_TYPE_LIST)
    #胸アクセサリ
    result += pack_pict_id(item_chestacce, common.ACCESORY_TYPE_LIST)
    #上半身+下半身
    if item_tops and item_tops.check_type(common.ONEPIECE_TYPE_LIST):
        result += pack_pict_id(item_tops, None)
        result += pack_int(0)
    else:
        result += pack_pict_id(item_tops, common.UPPER_TYPE_LIST)
        result += pack_pict_id(item_buttoms, common.LOWER_TYPE_LIST)
    #背中
    result += pack_pict_id(item_backpack, common.BACKPACK_TYPE_LIST)
    #右手装備
    result += pack_pict_id(item_right, common.RIGHT_TYPE_LIST)
    #左手装備
    result += pack_pict_id(item_left, common.LEFT_TYPE_LIST)
    #靴
    result += pack_pict_id(item_shoes, common.BOOTS_TYPE_LIST)
    #靴下
    result += pack_pict_id(item_socks, common.SOCKS_TYPE_LIST)
    #ペット
    result += pack_int(0)
    #effect #353+
    result += pack_pict_id(item_effect, common.EFFECT_TYPE_LIST)
...
def make_01ff(pc):
    ...
    result += pack_int(1) #射程
    result += pack_int(0) #不明
    result += pack_int(2) #mode
    result += pack_int(0) #mode
...
def make_05f2(option_list, title, show_cancel=0):
    """NPCのメッセージのうち、選択肢から選ぶもの
    選択結果はs05f3で通知する"""
    result = pack_str(title) #ウィンドウタイトル
    result += pack_array(pack_str, option_list) #選択肢 65以上でエラー
...
def make_13ec():
    return "\x32"+"\x00\x00\x00\x00\x00\x00\x00\x00"*50
...
def make_1be4(pc):
    """飛空庭ログイン #unfinished"""
    i = len(common.FLYGARDEN_ATTR_LIST)
    ...
    result += pack_array(pack_int, [
        ...
        for attr in common.FLYGARDEN_ATTR_LIST
    ]) #item_ids
    result += pack_array(pack_byte, (0,)*i) #colors
    result += pack_byte(0) #土台色？
    common.log(result.encode("hex"))
    return result
...
def make_1bef(pc):
    ...
    result += pack_short(pc.rawx)
    result += pack_short(pc.rawy) #高さ？
    result += pack_short(pc.rawz) #奥行き
    result += pack_short(pc.rawdir) #傾き？
    result += pack_short(pc.motion) #モーション
    result += pack_short(pc.y_rotate) #y軸回転？
    result += pack_short(pc.z_rotate) #z軸回転？ 
    result += pack_short(pc.name) #名前
    return result
...
name_map = common.get_name_map(globals(), "make_")