...
class LaunchDataHandler:
    ...
    def handle_data(self, data_decode):
        ...
        while True:
            ...
            if not handler:
                common.log_error("[launch] unknow packet type",
                    data[:2].encode("hex"), data[2:].encode("hex"))
                return
            try:
                handler(self, data_io)
            except:
                common.log_error("[launch] handle_data error:", data.encode("hex"))
                common.log_error(traceback.format_exc())
    
    def do_0001(self, data_io):
        #接続・接続確認
        data = data_io.read()
        common.log("[launch] eco version", unpack_unsigned_int(data[:4]))
        self.send("0002", data) #認証接続確認(s0001)の応答
        self.send("001e", self.word_front+self.word_back) #PASS鍵
        common.log("[launch] send word",
    ...
    def do_001f(self, data_io):
        ...
        common.log("[launch]", "login", username, password_sha1)
...
LaunchDataHandler.name_map = common.get_name_map(LaunchDataHandler.__dict__, "do_")