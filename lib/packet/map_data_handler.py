...
class MapDataHandler:
    def __init__(self):
        ...
        self.word_front = pack_unsigned_int(
            common.randint(0, common.RANGE_INT[1])
        )
    ...
    def send(self, *args):
        self.send_packet(common.encode(packet.make(*args), self.rijndael_obj))
    ...
    def send_map(self, *args):
        with self.pc.lock:
            if not self.pc.map_obj:
                common.log_error("[ map ] send_map: not self.pc.map_obj", self.pc)
                return
    ...
    def handle_data(self, data_decode):
        ...
        data_decode_io = common.stringio(data_decode)
        while True:
            data = io_unpack_short_raw(data_decode_io)
            if not data:
                break
            data_io = common.stringio(data)
            ...
            if not handler:
                common.log_error("[ map ] unknow packet type",
                    data[:2].encode("hex"), data[2:].encode("hex"))
                return
            try:
                handler(self, data_io)
            except:
                common.log_error("[ map ] handle_data error:", data.encode("hex"))
                common.log_error(traceback.format_exc())
                if data_type in DATA_TYPE_CANCEL_TRADE_WHEN_EXCEPT:
                    try:
                        self.pc.cancel_trade()
                    except:
                        common.log_error("[ map ] cancel trade when except error:")
                        common.log_error(traceback.format_exc())
    ...
    def do_000a(self, data_io):
        #接続・接続確認
        data = data_io.read()
        common.log("[ map ] eco version", unpack_unsigned_int(data[:4]))
        self.send("ffff")
        self.send("000b", data)
        self.send("000f", self.word_front+self.word_back)
        common.log("[ map ] send word",
    ...
    def do_0010(self, data_io):
        ...
        common.log("[ map ]", "login", username, password_sha1)
    ...
    def do_0037(self, data_io):
        ...
        common.log("[ map ] send pc info success")
    
    def do_11fe(self, data_io):
        #MAPワープ完了通知
        common.log("[ map ]", "map load")
        ...
        if usermaps.map_id_in_range_flygarden(self.pc.map_obj.map_id):
            ...
            for i, attr in enumerate(common.FLYGARDEN_ATTR_LIST):
                #飛空庭に装飾品を装着・解除するの結果
    ...
    def do_001e(self, data_io):
        ...
        common.log("[ map ] logout")
    
    def do_001f(self, data_io):
        #ログアウト開始&ログアウト失敗
        if io_unpack_byte(data_io) == 0:
            common.log("[ map ] logout success")
        else:
            common.log("[ map ] logout failed")
    
    def do_11f8(self, data_io):
        #自キャラの移動
        if self.pc.attack:
            common.log("[ map ] stop attack")
    ...
    def do_020c(self, data_io):
        ...
        common.log("[ map ] request object id", obj_id)
    ...
    def do_09e7(self, data_io):
        ...
        if self.pc.dem_form_status():
            common.log("[ map ] set equip failed", self.pc)
    ...
    def do_09f7(self, data_io):
        #倉庫を閉じる
        common.log("[ map ] warehouse closed")
    ...
    def do_09fb(self, data_io):
        ...
        common.log("[ map ] take item from warehouse", item_iid, item_count)
        with self.pc.lock:
    ...
    def do_09fd(self, data_io):
        ...
        item_iid = io_unpack_int(data_io)
        item_count = io_unpack_short(data_io)
        common.log("[ map ] store item to warehouse", item_iid, item_count)
        with self.pc.lock:
    ...
    def do_0617(self, data_io):
        ...
        common.log("[ map ] npcshop / npcsell close")
    
    def do_0614(self, data_io):
        #NPCショップのアイテム購入
        common.log("[ map ] npcshop")
        with self.pc.lock:
            if self.pc.shop_open is None:
                common.log_error("do_0614: shop_open is None")
                return
        ...
        common.log("[ map ] item_buy_list", item_buy_list)
        ...
        for item_id, item_count in item_buy_list:
            if not item_count:
                common.log_error("do_0614 error: item_count is 0", item_count)
                continue
            if item_id not in shop_item_list:
                common.log_error("do_0614 error: item_id not in shop_item_list", 
                    item_id, shop_item_list)
                continue
            item = db.item.get(item_id)
            if not item:
                common.log_error("do_0614 error: item_id not exist", item_id)
                continue
    ...
    def do_0616(self, data_io):
        #ショップで売却
        common.log("[ map ] npcsell")
        with self.pc.lock:
            if self.pc.shop_open != 65535:
                common.log_error("do_0616: shop_open != 65535", self.pc.shop_open)
                return
        item_iid_list = io_unpack_array(io_unpack_int, data_io)
        item_count_list = io_unpack_array(io_unpack_int, data_io)
        item_sell_list = zip(item_iid_list, item_count_list)
        common.log("[ map ] item_sell_list", item_sell_list)
        with self.pc.lock:
            for item_iid, item_count in item_sell_list:
                if not item_count:
                    common.log_error("do_0616: not item_count", item_count)
                    continue
                if self.pc.in_equip(item_iid):
                    common.log_error("do_0616: in_equip(item_iid)", item_iid)
                    continue
                item = self.pc.item.get(item_iid)
                if not item:
                    common.log_error("do_0616: not item", item_iid)
                    continue
                if item.count < item_count:
                    common.log_error("do_0616: item.count < item_count")
                    continue
    ...
    def do_0f9f(self, data_io):
        ...
        if not monster:
            common.log_error("[ map ] monster id %s not exist"%monster_id)
            return
        common.log("[ map ] attack monster id %s"%monster_id)
        with self.pc.lock:
            self.pc.attack = True
    ...
    def do_1d4c(self, data_io):
        ...
        with self.pc.lock and p.lock:
            rawdir = common.get_angle_from_coord(self.pc.x, self.pc.y, p.x, p.y)
            p_rawdir = common.get_angle_from_coord(p.x, p.y, self.pc.x, self.pc.y)
    ...
    def do_0a0a(self, data_io):
        ...
        p = users.get_pc_from_id(target_id)
        if not p:
            common.log("[ map ] trade ask: target not exist", target_id)
            self.send("0a0b", -5) #trade ask result, 相手が見つかりません
            return
        with self.pc.lock and p.lock and p.user.lock:
            if not p.online:
                common.log("[ map ] trade ask: target not online", target_id)
                self.send("0a0b", -5) #trade ask result, 相手が見つかりません
                return
    ...
    def do_0a0d(self, data_io):
        ...
        if not self.pc.trade_target_id:
            common.log("[ map ] trade answer: not self.pc.trade_target_id")
            return
    ...
    def do_07d0(self, data_io):
        ...
        iid = io_unpack_int(data_io)
        count = io_unpack_short(data_io)
        common.log("[ map ] put item: iid", iid, "count", count)
        with self.pc.lock:
            if self.pc.trade:
            ...
            if err:
                ...
                return
            if not item:
                common.log("[ map ] put item: not item but err = 0", iid, count)
                return
    ...
    def do_07e4(self, data_io):
        ...
        common.log("[ map ] pick up item: mapitem_id", mapitem_id)
        if len(self.pc.item) >= env.MAX_ITEM_STOCK:
    ...
    def do_1e7d(self, data_io):
        ...
        if self.pc.dem_form_change(status):
            common.log("[ map ] dem form change success")
        else:
            common.log("[ map ] dem form change failed")
    ...
    def do_1e87(self, data_io):
        ...
        if self.pc.dem_form_status():
            ...
            common.log("[ map ] set dem parts failed", self.pc)
            self.send("09e8", iid, -1, -2, 1) #アイテム装備
...
MapDataHandler.name_map = common.get_name_map(MapDataHandler.__dict__, "do_")