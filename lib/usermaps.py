...
def get_usermap_from_map_id(i):
    with usermap_list_lock:
        if map_id_in_range_flygarden(i):
            ...
            common.log_error("[umaps] get_usermap_from_map_id error:", i)

def set_usermap(pc, usermap_type, x, y):
    unset_usermap(pc)
    with pc.lock:
        with usermap_list_lock:
            i = common.make_id(usermap_list, MIN_USERMAP_ID)
        ...
        common.log("[umaps] set usermap id", i)
...
def unset_usermap(pc, logout=False):
    with pc.lock:
        if not pc.usermap_obj:
            return
        i = pc.usermap_obj.id
        common.log("[umaps] del usermap id", i)
        usermap_obj = pc.usermap_obj
        map_obj = common.get_map(usermap_obj.entrance_map_id)
        if not map_obj:
            common.log_error(
        ...
        for p in common.copy(usermap_obj.pc_list):
            if logout and p == pc:
