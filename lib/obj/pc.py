...
class PC:
    ...
    def _get_item_part(self, iid):
        ...
        for equip in (self.equip_std, self.equip_dem):
            if iid == equip.item['head']:
                if item.check_type(common.HEAD_TYPE_LIST):
                    return 0x06
                elif item.check_type(common.ACCESORY_HEAD_TYPE_LIST):
                    return 0x07
            elif iid == equip.item['face']:
                if item.check_type(common.FULLFACE_TYPE_LIST):
                    return 0x06 #0x08 before ver315
                elif item.check_type(common.ACCESORY_FACE_TYPE_LIST):
                    return 0x08 #0x09 before ver315
            elif iid == equip.item['chestacce']:
                return 10
            elif iid == equip.item['tops']:
                return 11
            elif iid == equip.item['bottoms']:
                return 12
            elif iid == equip.item['backpack']:
                return 13
            elif iid == equip.item['right']:
                return 14
            elif iid == equip.item['left']:
                return 15
            elif iid == equip.item['shoes']:
                return 16
            elif iid == equip.item['socks']:
                return 17
            elif iid == equip.item['pet']:
                return 18
            elif iid == equip.item['effect']:
                return 19
        return 0x02 #body
    ...
    def _set_map(self, map_id=None):
        ...
        if not map_obj:
            return False
        #common.log(self, "set_map", map_obj)
        with self.user.lock:
            self.unset_pet()
            if map_id:
    ...
    def set_visible(self, visible):
        with self.lock:
            ...
            common.log("[ pc  ] set visible", bool(visible))
    ...
    def _set_equip(self, iid):
        item = self.item.get(iid)
        if not item:
            common.log_error("[ pc  ] set_equip iid %d not exist"%iid, self)
            return False
        unset_iid_list = []
        set_part = 0
        if item.check_type(common.HEAD_TYPE_LIST): #頭
            unset_iid_list.append(self.equip.item["head"])
            self.equip.item["head"] = iid
            set_part = 6
        elif item.check_type(common.ACCESORY_HEAD_TYPE_LIST): #頭
            unset_iid_list.append(self.equip.item["head"])
            self.equip.item["head"] = iid
            set_part = 7
        elif item.check_type(common.FULLFACE_TYPE_LIST): #顔
            unset_iid_list.append(self.equip.item["face"])
            self.equip.item["face"] = iid
            set_part = 6 #8 before ver315
        elif item.check_type(common.ACCESORY_FACE_TYPE_LIST): #顔
            unset_iid_list.append(self.equip.item["face"])
            self.equip.item["face"] = iid
            set_part = 8 #9 before ver315
        elif item.check_type(common.ACCESORY_TYPE_LIST): #胸アクセサリ
            unset_iid_list.append(self.equip.item["chestacce"])
            self.equip.item["chestacce"] = iid
            set_part = 10
        elif item.check_type(common.ONEPIECE_TYPE_LIST): #...
            unset_iid_list.append(self.equip.item["tops"])
            unset_iid_list.append(self.equip.item["bottoms"])
            self.equip.item["tops"] = iid
            self.equip.item["bottoms"] = 0
            set_part = 11
        elif item.check_type(common.UPPER_TYPE_LIST): #上半身
            unset_iid_list.append(self.equip.item["tops"])
            self.equip.item["tops"] = iid
            set_part = 11
        elif item.check_type(common.LOWER_TYPE_LIST): #下半身
            item_tops = self.item[self.equip.item["tops"]]
            if item_tops and item_tops.check_type(common.ONEPIECE_TYPE_LIST):
                unset_iid_list.append(self.equip.item["tops"])
                self.equip.item["tops"] = 0
            unset_iid_list.append(self.equip.item["bottoms"])
            self.equip.item["bottoms"] = iid
            set_part = 12
        elif item.check_type(common.BACKPACK_TYPE_LIST): #背中
            unset_iid_list.append(self.equip.item["backpack"])
            self.equip.item["backpack"] = iid
            set_part = 13
        elif item.check_type(common.RIGHT_TYPE_LIST): #右手装備
            unset_iid_list.append(self.equip.item["right"])
            self.equip.item["right"] = iid
            set_part = 14
        elif item.check_type(common.LEFT_TYPE_LIST): #左手装備
            unset_iid_list.append(self.equip.item["left"])
            self.equip.item["left"] = iid
            set_part = 15
        elif item.check_type(common.BOOTS_TYPE_LIST): #靴
            unset_iid_list.append(self.equip.item["shoes"])
            self.equip.item["shoes"] = iid
            set_part = 16
        elif item.check_type(common.SOCKS_TYPE_LIST): #靴下
            unset_iid_list.append(self.equip.item["socks"])
            self.equip.item["socks"] = iid
            set_part = 17
        elif item.check_type(common.PET_TYPE_LIST): #ペット
            unset_iid_list.append(self.equip.item["pet"])
            self.unset_pet()
            self.equip.item["pet"]= iid
            self.set_pet()
            set_part = 18
        elif item.check_type(common.EFFECT_TYPE_LIST): #effect
            unset_iid_list.append(self.equip.item["effect"])
            self.equip.item["effect"] = iid
            set_part = 19
        else: #装備しようとする装備タイプが不明の場合
            common.log_error("[ pc  ] set_equip unknow item type", item)
            self.map_send("09e8", iid, -1, -2, 1) #アイテム装備
            return False
        common.log("[ pc  ] set_equip", item)
    ...
    def _unset_equip(self, iid, part):
        if iid == 0:
            common.log_error("[ pc  ] unset_equip iid == 0", self)
            return False
        if iid not in self.item:
            common.log_error("[ pc  ] unset_equip iid %d not exist"%iid, self)
            return False
        if self.equip.item["pet"]== iid:
            self.unset_pet()
        for attr in common.EQUIP_ATTR_LIST:
            if getattr(self.equip, attr) == iid:
                setattr(self.equip, attr, 0)
        common.log_error("[ pc  ] unset_equip iid %d"%iid)
    ...
    def item_append(self, item, place=0x02):
        ...
        #0x02: body.;p-
        with self.lock:
            item_iid = common.make_id(self.sort.item+self.warehouse)
    ...
    def warehouse_append(self, item):
        ...
        with self.lock:
            ...
            self.warehouse.append(item_iid)
            if self.online:
                #倉庫インベントリーデータ
    ...
    def get_trade_target(self):
        if not self.trade_target_id:
            return None
        if not self.online:
            common.log("[ pc  ] get_trade_target: not self.online")
            return False
        p = users.get_pc_from_id(self.trade_target_id)
        if not p:
            common.log("[ pc  ] get_trade_target: not p")
            return False
        if not p.online:
            common.log("[ pc  ] get_trade_target: not p.online")
            return False
        if self.map_id != p.map_id:
            common.log("[ pc  ] get_trade_target: self.map_id != p.map_id")
            return False
        return p
    ...
    def set_trade_list(self, trade_gold, trade_list):
        with self.lock and self.user.lock:
            ...
            common.log("[ pc  ] self.trade_gold", self.trade_gold)
            common.log("[ pc  ] self.trade_list", self.trade_list)
    ...
    def set_trade_return(self):
        ...
        with self.lock and self.user.lock:
            if not self.online:
                return
            common.log("[ pc  ] set_trade_return")
    ...
    def set_trade_finish(self):
        with self.lock and self.user.lock:
            if not self.online:
                return
            common.log("[ pc  ] set_trade_finish")
    ...
    def get_status(self, LV, STR, DEX, INT, VIT, AGI, MAG):
        def get_base_status(self):
            ...
            status.maxhp = (VIT*3+(VIT/5)**2+LV*2+(LV/5)**2+50)
            status.maxmp = (MAG*3+LV+(LV/9)**2+30)
            status.maxsp = (INT+VIT+LV+(LV/9)**2+20)
        ...
        def get_job_status(self):
            ...
            if not job:
                common.log_error("[ pc  ] unknow job id:", self.job)
                return
            status.maxhp = (status.maxhp*job.hp_rate)
            status.maxmp = (status.maxmp*job.mp_rate)
            status.maxsp = (status.maxsp*job.sp_rate)
        ...
        def get_equip_status(self):
            ...
            for item in self.get_equip_list(): #filter include False
                ...
                status.maxhp += (item.hp)
                status.maxmp += (item.mp)
                status.maxsp += (item.sp)
                status.maxpayl += int(item.payl_add/10)
                status.maxcapa += int(item.capa_add/10)
