...
def item_loads(s):
    l = dumpobj.loads(s)
    i = common.get_item(l["item_id"])
    i.__dict__.update(l)
    return i

def save(self):
    data = common.get_config()
    ...
    data.set("sort", "item", common.list_to_str(self.sort.item))
    data.set("sort", "warehouse", common.list_to_str(self.sort.warehouse))
    #"iid = id,count"
    ...
    for key, value in self.var.iteritems():
        try:
            ...
            common.log_error("[ pc  ] dump var error", self, key, value)
            common.log_error(traceback.format_exc())
            dump = str(value)
        data.set("var", str(key), dump)
    #"skill_id,skill_id,skill_id, ..."
    data.add_section("skill")
    data.set("skill", "list", common.list_to_str(self.skill_list))
    #
    data.write(open(self.path, "wb", base=env.USER_DIR))

def load(self):
    data = common.get_config(self.path, base=env.USER_DIR)
...
def load_1_0_0(self, data):
    ...
    self.item = {}
    self.sort.item = common.str_to_list(data.get("sort", "item"))
    for i in self.sort.item:
        if i <= 0:
            common.log_error("[ pc  ] item iid <= 0", self)
        itemdata = common.str_to_list(data.get("item", str(i)))
        item = common.get_item(itemdata[0])
        item.count = itemdata[1]
        self.item[i] = item
    #{item_iid: item_object, ...}
    self.warehouse = {}
    self.sort.warehouse = common.str_to_list(data.get("sort", "warehouse"))
    for i in self.sort.warehouse:
        if i <= 0:
            common.log_error("[ pc  ] warehouse iid <= 0", self)
        itemdata = common.str_to_list(data.get("warehouse", str(i)))
        item = common.get_item(itemdata[0])
        item.count = itemdata[1]
    ...
    if data.has_section("var"):
        for key in data.options("var"):
            try:
                ...
                common.log_error("[ pc  ] load var error", self, key)
                common.log_error(traceback.format_exc())
    #[skill_id, ...]
    self.skill_list = common.str_to_list(data.get("skill", "list"))
