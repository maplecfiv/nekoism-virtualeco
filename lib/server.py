...
if USE_NULL_KEY:
    ...
    GENERATOR = 3
    PRIME = common.get_prime()
    PRIVATE_KEY = common.get_private_key()
    PUBLIC_KEY = common.get_public_key(GENERATOR, PRIVATE_KEY, PRIME)
    #get bytes
    PRIME_BYTES = common.int_to_bytes(PRIME)
    PUBLIC_KEY_BYTES = common.int_to_bytes(PUBLIC_KEY)
    #common.log("prime:", PRIME_BYTES, "\nlength:", len(PRIME_BYTES))
    #common.log("public key:", PUBLIC_KEY_BYTES, "\nlength:", len(PUBLIC_KEY_BYTES))
...
class StandardServer(threading.Thread):
    ...
    def _shutdown(self):
        if not self.running:
            return
        common.log("[ srv ] shutdown", self)
        self.running = False
        self.socket.close()
        while self.client_list:
            client = self.client_list[0]
            common.log("[ srv ] shutdown", client)
        ...
        with self.client_list_lock:
            for client in self.client_list:
                if ip == client.src_address[0]:
                    ip_count += 1
        common.log("[ srv ] src: %s ip_count: %s"%(src, ip_count))
...
class StandardClient(threading.Thread):
    ...
    def run(self):
        try:
            ...
            common.log_error(traceback.format_exc())
            self.stop()
        common.log("[ srv ] quit", self)
    
    def send_packet(self, packet):
        #common.log("[ srv ] send", packet.encode("hex"))
        with self.send_lock:
    ...
    def recv_key(self):
        ...
        client_public_key = common.bytes_to_int(client_public_key_bytes)
        #common.log("[ srv ] client key:", client_public_key_bytes)
        #common.log("[ srv ] length:", len(client_public_key_bytes))
        self.recv_key = True
        if USE_NULL_KEY:
            ...
            #get share key
            share_key_bytes = common.get_share_key_bytes(
                client_public_key, PRIVATE_KEY, PRIME)
            common.log("[ srv ] share key:", share_key_bytes)
            #common.log("[ srv ] length:", len(share_key_bytes))
            #get rijndael key (str)
            self.rijndael_key = common.get_rijndael_key(share_key_bytes)
            self.rijndael_obj = rijndael.rijndael(
                self.rijndael_key, block_size=16
            )
            self.rijndael_obj.lock = threading.RLock()
        common.log("[ srv ] rijndael key:", self.rijndael_key.encode("hex"))
    
    def handle_packet(self):
        packet = self.recv_enc_packet()
        try:
            self.handle_data(common.decode(packet, self.rijndael_obj))
        except:
            common.log_error(traceback.format_exc())
...
def assert_address_not_used():
    common.log_line("[ srv ] server.assert_address_not_used ... ")
    common.assert_address_not_used((env.SERVER_BROADCAST_ADDR, env.LAUNCH_SERVER_PORT))
    common.assert_address_not_used((env.SERVER_BROADCAST_ADDR, env.LOGIN_SERVER_PORT))
    common.assert_address_not_used((env.SERVER_BROADCAST_ADDR, env.MAP_SERVER_PORT))
    common.assert_address_not_used((env.SERVER_BROADCAST_ADDR, env.WEB_SERVER_PORT))
    common.log("done.")
...
def load():
    ...
    common.log("[ srv ] start launch server with\t%s:%d"%launchserver_bind_addr)
    launchserver = LaunchServer(launchserver_bind_addr)
    common.log("[ srv ] start login server with\t%s:%d"%loginserver_bind_addr)
    loginserver = LoginServer(loginserver_bind_addr)
    common.log("[ srv ] start map server with\t%s:%d"%mapserver_bind_addr)
