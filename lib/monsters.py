...
def spawn(monster_id, map_id, x, y):
    error = None
    monster = common.get_monster(monster_id)
    if not monster:
    ...
    with monster_list_lock and monster.lock:
        monster_id = common.make_id(monster_id_list, MIN_MONSTER_ID)
    ...
    if error:
        delete(monster)
        return error
    common.log("[monster] spawn monster id %s"%(monster.id))
...
def delete(monster):
    with monster_list_lock and monster.lock:
        ...
        common.log("[monster] delete monster id %s"%(monster.id))
        monster.reset()
    del monster
...
def set_skill_attack_result(monster, damage, ml, dl, cl, el):
...
def skill_attack_monster(pc, monster, damage, skill_id, skill_lv):
    ml, dl, cl, el = [], [], [], []
...
def skill_attack_coord(pc, x, y, xyr, damage, skill_id, skill_lv):
    ...
    with pc.lock and pc.map_obj.lock:
        for m in pc.map_obj.monster_list:
            if not common.coord_in_range(m.x, m.y, x, y, xyr):
                continue
