...
class User:
    ...
    def load(self):
        cfg = common.get_config(os.path.join(
        ...
        if self.pc_list:
            common.log_error("[users] ERROR: pc_list already load.", self)
            return
...
def make_new_user(user_name, password, delpassword):
    if get_user_from_name(user_name):
        return False
    with user_list_lock:
        user_id = common.make_id(user_id_set, MIN_USER_ID)
        if user_id >= MAX_USER_ID:
            common.log_error("[users] ERROR: user_id [%s] >= MAX_USER_ID"%user_id)
            return False
        common.log("[users] add user id", user_id)
    ...
    with user.lock:
        ...
        with user_list_lock:
            try:
                ...
                common.log("[users] remove user id", user.user_id)
                user_id_set.remove(user.user_id)
                for p in user.pc_list:
                    if p:
                        common.log("[users] remove pc id", p.id)
...
def modify_password(user_name,
    ...
    with user.lock:
        ...
        #common.log(old_password_md5, old_delete_password_md5)
...
def make_new_pc(user, num, name, race, gender, hair, hair_color, face):
    ...
    if os.path.exists(path):
        return False
    with user_list_lock:
        pc_id = common.make_id(pc_id_set, MIN_PC_ID)
        if pc_id >= MAX_PC_ID:
            common.log_error("[users] ERROR: pc_id [%s] >= MAX_PC_ID"%pc_id)
            return False
        common.log("[users] add pc id", pc_id)
        pc_id_set.add(pc_id)
    cfg = common.get_config()
    cfg.add_section("main")
...
def backup_user_data():
    try:
        zip_path = os.path.join(env.USER_BAK_DIR, "%s.zip"%common.get_today())
        if os.path.exists(zip_path):
            return
        if not os.path.exists(env.USER_BAK_DIR):
            os.mkdir(env.USER_BAK_DIR)
        common.save_zip(env.USER_DIR, zip_path, env.USER_DIR, env.USER_BAK_DIR)
    except:
        common.log_error("backup_user_data", traceback.format_exc())
...
def backup_user_data_every_day():
    ...
    if not backup_user_data_every_day_thread_running:
        ...
        common.start_thread(backup_user_data_every_day_thread, ())
    ...
    try:
        for p in get_online_pc_list():
            try:
                ...
                common.log_error(traceback.format_exc(), p)
    except:
        common.log_error(traceback.format_exc())
...
def save_user_data_every_min():
    ...
    if not save_user_data_every_min_thread_running:
        ...
        common.start_thread(save_user_data_every_min_thread, ())
def save_user_data_atexit():
    for p in get_online_pc_list():
        try:
            common.log("[users] save atexit", p)
            p.save()
        except:
            common.log_error(traceback.format_exc(), p)
    ...
    with user_list_lock:
        for name in os.listdir(env.USER_DIR):
            try:
                ...
                common.log_error("load error:", name)
                raise
    #for user in get_user_list():
    #    common.log(user)
    #for p in get_pc_list():
